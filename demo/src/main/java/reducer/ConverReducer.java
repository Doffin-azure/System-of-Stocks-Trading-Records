...
public class ConverReducer extends Reducer<Text, Text, Text, Text> {
    ...
    public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
        if (firstOutput) {
            ...
            context.write(null, new Text(header));  // 输出表头
        ...
        // 计算主力净流入、主力流入和主力流出
        double inflow = buy_largeAmount + buy_extraLargeAmount; // 主力流入
        double outflow = sell_largeAmount + sell_extraLargeAmount; // 主力流出
        double netFlow = inflow - outflow;
        String timeRange = key.toString();
        timeRange = timeRange.replace("to", " to ");  // 修改时间范围的格式
        // 输出结果
